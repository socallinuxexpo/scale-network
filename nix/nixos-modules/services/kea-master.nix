...
in
{
  ...
  config = mkIf cfg.enable {
    ...
    systemd.services.kea-dhcp4-server.serviceConfig.ExecStart =
      lib.mkForce "${config.services.kea.package}/bin/kea-dhcp4 -X -c /etc/kea/dhcp4-server.conf";

    systemd.services.kea-dhcp6-server.serviceConfig.ExecStart =
      lib.mkForce "${config.services.kea.package}/bin/kea-dhcp6 -X -c /etc/kea/dhcp6-server.conf";
