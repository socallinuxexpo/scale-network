...
do
  ...
  do
    (echo -n "$mac "; cat /sys/kernel/debug/ieee80211/phy0/net*/stations/$mac/airtime| awk '{printf "%s ", $0} END{print ""}') |logger -p local7.info -t "stats-24-$mac"
    (echo -n "$mac "; cat /sys/kernel/debug/ieee80211/phy0/net*/stations/$mac/*capa| awk '{printf "%s ", $0} END{print ""}') |logger -p local7.info -t "stats-24-$mac-capa"
    (echo -n "$mac "; cat /sys/kernel/debug/ieee80211/phy0/net*/stations/$mac/aqm| awk '{printf "%s ", $0} END{print ""}') |logger -p local7.info -t "stats-24-$mac-.capa"
  done
  find /sys/kernel/debug/ieee80211/phy1 -name airtime |cut -f 9 -d '/' |while read mac
  do
     (echo -n "$mac "; cat /sys/kernel/debug/ieee80211/phy1/net*/stations/$mac/airtime| awk '{printf "%s ", $0} END{print ""}') |logger -p local7.info -t "stats-5-$mac"
     (echo -n "$mac "; cat /sys/kernel/debug/ieee80211/phy1/net*/stations/$mac/*capa| awk '{printf "%s ", $0} END{print ""}') |logger -p local7.info -t "stats-5-$mac-capa"
     (echo -n "$mac "; cat /sys/kernel/debug/ieee80211/phy1/net*/stations/$mac/aqm| awk '{printf "%s ", $0} END{print ""}') |logger -p local7.info -t "stats-5-$mac-aqm"
  done
